...
!(function (y, n, a) {
	...
	var t,
		...
		s = {
			contextActions: {
				...
				edit_img: function (e) {
					...
					n.attr("data-name", a),
						...
						(i = r),
						imageEditor
							.loadImageFromURL(i, "SampleImage")
							.then(function (e) {
				...
				select: function (e) {
					...
					(1 == jQuery("#return_relative_url").val() &&
						(t = (t = t.replace(
							jQuery("#base_url").val(),
							""
						)).replace(jQuery("#cur_dir").val(), "")),
					(a =
						1 == jQuery("#popup").val()
							? window.opener
							: window.parent),
					"" != r)
						? 1 == jQuery("#crossdomain").val()
							...
							: (jQuery("#" + r, a.document)
									...
									.trigger("change"),
							  "function" ==
									typeof a.responsive_filemanager_callback &&
				...
				chmod: function (e) {
					!(function (e) {
						...
						y.ajax({
							...
							data: { path: i, permissions: a, folder: n },
						}).done(function (e) {
							bootbox.dialog(
								e,
								[
									...
									},
									{
										...
										callback: function () {
											var a = "-";
											jQuery("#u_4").is(":checked")
												? (a += "r")
												: (a += "-"),
												jQuery("#u_2").is(":checked")
													? (a += "w")
													: (a += "-"),
												jQuery("#u_1").is(":checked")
													? (a += "x")
													: (a += "-"),
												jQuery("#g_4").is(":checked")
													? (a += "r")
													: (a += "-"),
												jQuery("#g_2").is(":checked")
													? (a += "w")
													: (a += "-"),
												jQuery("#g_1").is(":checked")
													? (a += "x")
													: (a += "-"),
												jQuery("#a_4").is(":checked")
													? (a += "r")
													: (a += "-"),
												jQuery("#a_2").is(":checked")
													? (a += "w")
													: (a += "-"),
												jQuery("#a_1").is(":checked")
													? (a += "x")
													: (a += "-");
											var e = jQuery(
												"#chmod_form #chmod_value"
											).val();
											if ("" != e && void 0 !== e) {
												var t = jQuery(
								...
								[
									...
									},
									{
										...
										callback: function () {
											...
											window.editor &&
												"function" ==
													typeof window.editor
														.getData &&
												(e = window.editor.getData()),
												y
													.ajax({
														type: "POST",
			...
			makeContextMenu: function () {
				...
				y.contextMenu({
					selector:
						"figure:not(.back-directory), .list-view2 figure:not(.back-directory)",
					autoHide: !0,
					build: function (t) {
						t.addClass("selected");
						var e = {
			...
			bindGridEvents: function () {
				...
				e.on("click", ".modalAV", function (e) {
					...
					t.removeData("modal"),
						t.modal({ backdrop: "static", keyboard: !1 }),
						a.hasClass("audio")
							? r.css("height", "80px")
							: r.css("height", "345px"),
						y.ajax({
					...
					jQuery("ul.grid").on("click", "div.box", function (e) {
						...
						if (0 !== a.length) r(a);
							...
							0 !== t.length &&
								(document.location = jQuery(t).prop("href"));
						}
					});
			},
			makeFilters: function (e) {
				jQuery("#filter-input")
					.on("keyup", function () {
						(c = 0),
							y(".selection:checkbox").removeAttr("checked"),
							s.updateMultipleSelectionButtons(),
								...
								r(function () {
									jQuery("li", "ul.grid ").each(function () {
										...
										y
											.ajax({
												url:
													"ajax_calls.php?action=filter&type=" +
													a,
											})
											.done(function (e) {
												"" != e && bootbox.alert(e);
						...
						function (e, a) {
							a.formData = {
								fldr:
									jQuery("#sub_folder").val() +
									jQuery("#fldr_value").val() +
									(a.files[0].relativePath ||
										a.files[0].webkitRelativePath ||
										""),
							};
						}
					),
			...
			makeSort: function (r) {
				jQuery("input[name=radio-sort]").on("click", function () {
					...
					(c = 0),
						y(".selection:checkbox").removeAttr("checked"),
						s.updateMultipleSelectionButtons(),
						t.removeClass("btn-inverse"),
						t.find("i").removeClass("icon-white"),
				...
				jQuery(".sorter").on("click", function () {
					var e = jQuery(this);
					(i =
						jQuery("#sort_by").val() !== e.attr("data-sort") ||
						0 == i),
						r
							? (y.ajax({
									url:
										"ajax_calls.php?action=sort&sort_by=" +
										e.attr("data-sort") +
										"&descending=" +
										(i ? 1 : 0),
							  }),
							  _(i, "." + e.attr("data-sort")),
							  jQuery(" a.sorter")
		...
		if (((n.user = 0), (n.group = 0), void (n.all = 0) !== e && 1 == e)) {
			...
			jQuery("#chmod_form input:checkbox:checked").each(function () {
				...
				jQuery("#chmod_form #chmod_value").val(
					n.user.toString() + n.group.toString() + n.all.toString()
				);
	}
	function u(e, a) {
		var t;
		("copy" != a && "cut" != a) ||
			((t = e.closest("figure").attr("data-path")),
			y
				.ajax({
					type: "POST",
					url: "ajax_calls.php?action=copy_cut",
					data: { path: t, sub_action: a },
				})
				.done(function (e) {
					"" != e
						? bootbox.alert(e)
						: (jQuery("#clipboard").val("1"), f(!0));
				}));
	}
	function d(t) {
		bootbox.confirm(
	...
	function f(e) {
		1 == e
			? jQuery(".paste-here-btn, .clear-clipboard-btn").removeClass(
					"disabled"
			  )
			: jQuery(".paste-here-btn, .clear-clipboard-btn").addClass(
					"disabled"
			  );
	}
	function p(e) {
		...
		if (0 < t.val()) {
			if (1 == t.val())
				...
				0 == i && ((i = 1), jQuery("h4").css("font-size", 12)),
					(a = Math.floor(a / i - 3)),
					jQuery("ul.grid li, ul.grid figure").css("width", a);
			}
			r.hide();
		} else n.touch && r.show();
	}
	function v() {
		...
		0 == jQuery("#view").val() &&
			(1 == e.attr("toggle")
				? (e.attr("toggle", 0),
				  e.animate({ top: "0px" }, { queue: !1, duration: 300 }))
				: (e.attr("toggle", 1),
				  e.animate({ top: "-30px" }, { queue: !1, duration: 300 })));
	}
	function m(e) {
	...
	function j() {
		1 == jQuery("#popup").val()
			...
			: ("function" == typeof parent.jQuery(".modal:has(iframe)").modal &&
			  ...
			  void 0 !== parent.jQuery && parent.jQuery
	...
	function Q(e) {
		return null != e
			? ("true" == jQuery("#transliteration").val() &&
			  ...
			  "true" == jQuery("#lower_case").val() && (e = e.toLowerCase()),
			  (e = (e = (e = (e = (e = e.replace('"', "")).replace(
					"'",
					""
	...
	function _(e, t) {
		...
		a.each(function () {
			var e = jQuery(this),
				a = e.find(t).val();
			if (y.isNumeric(a))
				for (a = parseFloat(a); void 0 !== i[a] && i[a]; )
					a = parseFloat(parseFloat(a) + parseFloat(0.001));
			...
			r.each(function () {
				var e = jQuery(this),
					a = e.find(t).val();
				if (y.isNumeric(a))
					for (a = parseFloat(a); void 0 !== l[a] && l[a]; )
						a = parseFloat(parseFloat(a) + parseFloat(0.001));
				else a = a + "a" + e.find("h4 a").attr("data-file");
				(l[a] = e.html()), o.push(a);
			}),
			...
			y.isNumeric(o[0])
				...
				: o.sort(),
			e && (n.reverse(), o.reverse()),
			a.each(function (e) {
				jQuery(this).html(i[n[e]]);
			}),
			r.each(function (e) {
				jQuery(this).html(l[o[e]]),
					jQuery(this).attr(
						"data-name",
						jQuery(this).children().attr("data-name")
					);
			});
	}
	function b() {
	...
	jQuery(document).ready(function () {
		...
		if (
			(jQuery(".ff-container").on("click", ".checkmark", function (e) {
				e.stopPropagation(),
					jQuery(this).parent().find("input").is(":checked")
						? c--
						: c++,
					s.updateMultipleSelectionButtons();
			}),
		...
		s.makeSort(t),
			...
			jQuery("#change_lang_btn").on("click", function () {
				y.ajax({
					...
					bootbox.dialog(
						e,
						[
							{ label: jQuery("#cancel").val(), class: "btn" },
							{
								label: jQuery("#ok").val(),
								class: "btn-inverse",
								callback: function () {
								...
								[
									...
									},
									{
										...
										callback: function () {
											var e =
													jQuery(
														"#create_text_file_name"
													).val() +
													jQuery(
														"#create_text_file_extension"
													).val(),
												a = jQuery(
													"#textfile_create_area"
												).val();
											if (null !== e) {
												e = Q(e);
												var t =
			...
			jQuery(".view-controller button").on("click", function () {
				...
				jQuery(".view-controller button").removeClass("btn-inverse"),
					...
					y
						.ajax({
							url:
								"ajax_calls.php?action=view&type=" +
								e.attr("data-value"),
						})
						.done(function (e) {
							"" != e && bootbox.alert(e);
						}),
					void 0 !== jQuery("ul.grid")[0] &&
						jQuery("ul.grid")[0] &&
						(jQuery("ul.grid")[0].className = jQuery(
							"ul.grid"
						)[0].className.replace(/\blist-view.*?\b/g, "")),
					void 0 !== jQuery(".sorter-container")[0] &&
				...
				jQuery("#view").val(a),
					jQuery("ul.grid").addClass("list-view" + a),
					jQuery(".sorter-container").addClass("list-view" + a),
					1 <= e.attr("data-value")
						? p(14)
			...
			jQuery(".clear-clipboard-btn").on("click", function () {
				0 == jQuery(this).hasClass("disabled") &&
					bootbox.confirm(
		...
		var u = function (a) {
			...
			return (
				jQuery(".selection:checkbox:checked:visible").each(function () {
					...
					a && (e = jQuery(this).closest("figure").attr("data-path")),
						t.push(e);
				}),
				t
			);
		...
		if (
						...
						function (e) {
							if (1 == e) {
								...
								t.text(parseInt(t.text()) - a.length),
									jQuery(
										".selection:checkbox:checked:visible"
									).each(function () {
			...
			r.on("mouseover", function () {
				0 == jQuery("#view").val() &&
					!1 ===
						jQuery("#main-item-container").hasClass(
							"no-effect-slide"
						) &&
					jQuery(this)
		...
		jQuery(window).resize(function () {
			...
			p(14),
			1 == jQuery("#clipboard").val() ? f(!0) : f(!1),
			jQuery("li.dir, li.file").draggable({
				...
				start: function (e, a) {
					jQuery(a.helper).addClass("ui-draggable-helper"),
						0 == jQuery("#view").val() &&
							jQuery("#main-item-container").addClass(
								"no-effect-slide"
							);
				},
				stop: function () {
					jQuery(this).removeClass("selected"),
						0 == jQuery("#view").val() &&
							jQuery("#main-item-container").removeClass(
								"no-effect-slide"
							);
				},
			}),
			...
			jQuery(document).on(
				...
				function () {
					...
					null == e.val().match(/^[0-7]{3}$/) &&
						(e.val(e.attr("data-def-value")), o(!0));
				}
			),
			(i = new LazyLoad()),
			new Clipboard(".btn");
			...
			if ((1 == o.length && (u = o[0]), "" != a)) {
				if (1 == jQuery("#crossdomain").val())
					...
					jQuery("#" + a, t.document)
						...
						0 == r
							? "function" ==
									typeof t.responsive_filemanager_callback &&
				...
				1 == jQuery("#crossdomain").val()
					...
					? (parent.tinymce.activeEditor.execCommand(
					  ...
					  parent.tinymce.activeEditor.windowManager.close(
							parent.tinymce.activeEditor.windowManager.params
								.mce_window_id
					  ))
					: (parent.tinymce.activeEditor.insertContent(i),
					  parent.tinymce.activeEditor.windowManager.close());
			}
		}),
		(apply_link = function (e, a) {
			...
			var i = m(e),
				n = JSON.stringify(i);
			// 20221212 eh
			if (parent["responsive_filemanager_callback"]) {
				parent.responsive_filemanager_callback(i[0], n, "link");
				return;
			}
			(1 == i.length && (n = i[0]), "" != a)
				? 1 == jQuery("#crossdomain").val()
		...
		(apply_img = function (e, a) {
			...
			// 20221212 eh
			if (parent["responsive_filemanager_callback"]) {
				parent.responsive_filemanager_callback(i[0], n, "img");
				return;
			}
		...
		(apply_video = function (e, a) {
			...
			// 20221212 eh
			if (parent["responsive_filemanager_callback"]) {
				parent.responsive_filemanager_callback(i[0], n, "video");
				return;
			}
			(1 == i.length && (n = i[0]), "" != a)
				? 1 == jQuery("#crossdomain").val()
		...
		(apply_none = function (e, a, t) {
			...
			// 20221212 eh
			if (parent["responsive_filemanager_callback"]) {
				parent.responsive_filemanager_callback(i[0], n, "none");
				return;
			}
			(1 == i.length && (n = i[0]), "" != a)
				? 1 == jQuery("#crossdomain").val()
					...
					: (jQuery("#" + a, t.document)
					  ...
					  j())
				: apply_any(i[0]);
			// originally only this:
			t.parent().find("form a")[1].click(),
				jQuery(".tip-right").tooltip("hide");
		}),
		(apply_any = function (e) {
			if (1 == jQuery("#crossdomain").val())
				window.parent.postMessage(
					{ sender: "responsivefilemanager", url: e, field_id: null },
					"*"
				);
				...
				var a =
					...
					(r = window.location.search.match(t)) && 1 < r.length
						? r[1]
						: null);
